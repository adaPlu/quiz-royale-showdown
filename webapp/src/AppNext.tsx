  ...
  return (
    <ArenaPage>
      ...
      <div className="stat-grid">
        <Stat label="POINTS" value={stats.totalPoints} /><Stat label="WINS" value={stats.wins} />
        <Stat label="MATCHES" value={stats.matchesPlayed} /><Stat label="BEST" value={stats.bestScore} />
      </div>
          ...
          return (
            <button key={mode} className={`mode-card ${info.accent}`} onClick={() => onPlay(mode)} disabled={joiningMode !== null}>
              <div className="mode-heading">
                {mode === "QUICK" ? <Bolt aria-hidden="true" /> : mode === "TOURNAMENT" ? <Trophy aria-hidden="true" /> : <Medal aria-hidden="true" />}
                <span><strong>{info.title}</strong><small>{info.tagline}</small></span>
              </div>
              <p>{info.description}</p>
              <span className="enter-label" aria-live="polite">{joiningMode === mode ? "FINDING ARENA…" : "ENTER ARENA →"}</span>
            </button>
          );
        })}
      </div>
...
function StoreScreen({ identity, onIdentity }: { identity: Identity; onIdentity: (identity: Identity) => void }) {
  ...
  async function purchase(itemId: string) {
    ...
    try {
      ...
      onIdentity(await refreshIdentity(identity));
    } catch (e) {
  ...
  return (
    <ArenaPage title="STORE" subtitle="Power-ups, passes and cosmetics">
      ...
      {!store && !error && <p className="muted" aria-live="polite">Loading inventory…</p>}
      <div className="item-grid">
        {store?.items.map((item) => (
          <article className="arena-card store-item" key={item.itemId}>
            <p className="eyebrow">{item.itemType}</p><h3>{item.displayName}</h3><p>{item.description}</p>
...
function SeasonScreen({ identity }: { identity: Identity }) {
  ...
  return (
    <ArenaPage title="SEASON" subtitle="Climb the current reward track">
      ...
      {season && <>
        <section className="arena-card season-hero"><Sparkles className="violet-text" size={34} aria-hidden="true" /><div><p className="eyebrow">CURRENT SEASON</p><h2>{season.season.name}</h2></div></section>
        <section className="arena-card"><div className="season-level"><strong>LEVEL {progress?.level}</strong><span>{progress?.xp} XP</span></div><div className="progress" role="progressbar" aria-valuenow={levelProgress} aria-valuemin={0} aria-valuemax={100}><span style={{ width: `${levelProgress}%` }} /></div><p className="muted">Tickets earned: {progress?.ticketsEarned ?? 0}</p></section>
      </>}
    </ArenaPage>
  );
}
...
function ProfileScreen({ identity, onIdentity, onMessage }: { identity: Identity; onIdentity: (identity: Identity) => void; onMessage: (message: string | null) => void }) {
  ...
  if (identity.kind === "guest") {
    async function submit(event: FormEvent<HTMLFormElement>) {
      event.preventDefault(); setBusy(true); onMessage(null);
      const form = new FormData(event.currentTarget);
      try {
        if (mode === "login") onIdentity(await login(String(form.get("identifier") ?? ""), String(form.get("password") ?? "")));
        else onIdentity(await register(String(form.get("username") ?? ""), String(form.get("email") ?? ""), String(form.get("password") ?? ""), identity));
      } catch (error) {
    ...
    return (
      <ArenaPage title="PROFILE" subtitle="Save your progress permanently">
        <div className="auth-switch" role="tablist" aria-label="Account action">
          <button role="tab" aria-selected={mode === "register"} className={mode === "register" ? "active" : ""} onClick={() => setMode("register")}><UserPlus size={18} aria-hidden="true" /> REGISTER</button>
          <button role="tab" aria-selected={mode === "login"} className={mode === "login" ? "active" : ""} onClick={() => setMode("login")}><LogIn size={18} aria-hidden="true" /> SIGN IN</button>
        </div>
        <form className="arena-card auth-form" onSubmit={submit}>
          {mode === "register" ? <><label>Username<input name="username" autoComplete="username" required minLength={3} /></label><label>Email<input name="email" type="email" autoComplete="email" required /></label></> : <label>Email or username<input name="identifier" autoComplete="username" required /></label>}
  ...
  return (
    <ArenaPage title="PROFILE" subtitle={identity.profile.username}>
      <section className="arena-card profile-card"><UserRound size={36} className="gold-text" aria-hidden="true" /><div><h2>{identity.profile.username}</h2><p>{identity.profile.email}</p></div></section>
      <div className="stat-grid"><Stat label="POINTS" value={stats.totalPoints} /><Stat label="WINS" value={stats.wins} /><Stat label="CORRECT" value={stats.correctAnswers} /><Stat label="RANK" value={stats.bestRank ?? "—"} /></div>
      <FriendsPanel identity={identity} onIdentity={onIdentity} />
...
function MatchScreen({ live, socket, connectionState, onExit }: { live: LiveMatch; socket: WebSocket | null; connectionState: ConnectionState; onExit: () => void }) {
  ...
  function answer(index: number) {
    if (!match.question || you.answerIndex != null || socket?.readyState !== WebSocket.OPEN || match.phase !== "QUESTION") return;
  ...
  function usePowerUp(powerUp: PowerUp) {
    if (socket?.readyState !== WebSocket.OPEN || match.phase !== "QUESTION") return;
    socket.send(JSON.stringify({ type: "USE_POWERUP", powerUp }));
  }

  useEffect(() => {
    function keyHandler(event: KeyboardEvent) {
      if (event.key === "Escape") { void onExit(); return; }
      const index = Number.parseInt(event.key, 10) - 1;
      if (index >= 0 && index <= 3) answer(index);
    }
    window.addEventListener("keydown", keyHandler);
    return () => window.removeEventListener("keydown", keyHandler);
  });

  const resultText = match.phase === "REVEAL" && you.answerIndex != null
    ? you.answerIndex === match.correctIndex ? "Correct!" : "Not this round."
    : null;

  return (
    <div className="match-page">
      <div className="match-topbar">
        <button onClick={onExit}>← EXIT</button><strong>{match.mode}</strong>
        <span className={`connection-pill ${connectionState}`} aria-live="polite">
          {connectionState === "live" ? <Wifi size={15} aria-hidden="true" /> : <WifiOff size={15} aria-hidden="true" />}
          {connectionState === "reconnecting" ? "RECONNECTING" : connectionState === "live" ? `${match.phase === "FINISHED" ? "FINAL" : `${seconds}s`}` : connectionState.toUpperCase()}
        </span>
      </div>
      <div className="phase-timer" aria-hidden="true"><span style={{ width: `${timerPercent}%` }} /></div>
      <div className="match-content">
        <div className="match-meta"><p className="eyebrow">ROUND {match.roundNumber} / {match.totalRounds}</p><span>{you.score} pts · streak {you.streak} · lives {you.lives}</span></div>
        ...
        {match.question && <>
          ...
          {you.availablePowerUps.length > 0 && match.phase === "QUESTION" && (
            <div className="powerup-row" aria-label="Available power-ups">
              {you.availablePowerUps.map((powerUp) => (
                <button key={powerUp} onClick={() => usePowerUp(powerUp)} disabled={socket?.readyState !== WebSocket.OPEN}>
                  {powerUp === "SHIELD" ? <Shield size={17} aria-hidden="true" /> : powerUp === "DOUBLE_DOWN" ? <Zap size={17} aria-hidden="true" /> : <Sparkles size={17} aria-hidden="true" />}
          ...
          <div className="answer-grid">
            {match.question.options.map((option, index) => {
              ...
              const wrongSelected = match.phase === "REVEAL" && selected && !correct;
              return (
                <button
                  key={`${match.question?.id}-${index}`}
                  className={`answer ${selected ? "selected" : ""} ${correct ? "correct" : ""} ${wrongSelected ? "wrong" : ""}`}
                  disabled={removed || you.answerIndex != null || match.phase !== "QUESTION"}
                  onClick={() => answer(index)}
                  aria-pressed={selected}
                  aria-label={`Answer ${index + 1}: ${removed ? "removed" : option}`}
                >
          ...
          {resultText && <p className={resultText === "Correct!" ? "answer-result correct-text" : "answer-result wrong-text"} role="status">{resultText}</p>}
        </>}
        <section className="arena-card standings" aria-labelledby="standings-title">
          <div className="season-level"><strong id="standings-title">STANDINGS</strong><span>{match.aliveCount} alive</span></div>
          {sortedPlayers.slice(0, 8).map((player, index) => (
            <div className={player.id === you.playerId ? "standing you" : "standing"} key={player.id}>
              <span>#{player.placement ?? index + 1} {player.name}{!player.alive ? " · OUT" : ""}</span><strong>{player.score}</strong>
            </div>
          ))}
        </section>
        {match.phase === "FINISHED" && <section className="finish-card"><Crown aria-hidden="true" /><h2>{match.winnerId === you.playerId ? "YOU WON" : `PLACEMENT #${you.placement ?? "—"}`}</h2><p>{you.score} final points</p><button className="primary-button" onClick={onExit}>RETURN HOME</button></section>}
        <p className="keyboard-hint">Keyboard: 1–4 answers · Esc exits</p>
      </div>
    </div>
  );
}

function powerUpLabel(powerUp: PowerUp): string {
  if (powerUp === "FIFTY_FIFTY") return "50:50";
  if (powerUp === "DOUBLE_DOWN") return "Double";
  return "Shield";
}

function ArenaPage({ children, title, subtitle }: { children: ReactNode; title?: string; subtitle?: string }) {
  return <div className="arena-page">{title && <header className="page-header"><p className="eyebrow">QUIZ ROYALE SHOWDOWN</p><h1>{title}</h1>{subtitle && <p>{subtitle}</p>}</header>}{children}</div>;
}

function LockedPage({ title, message }: { title: string; message: string }) {
  return <ArenaPage title={title}><section className="arena-card locked-card"><Crown size={34} aria-hidden="true" /><h2>Account required</h2><p>{message}</p><small>Open PROFILE below to register or sign in.</small></section></ArenaPage>;
}
...
function LoadingScreen({ message }: { message: string }) {
  return <div className="loading" role="status" aria-live="polite"><Crown size={48} aria-hidden="true" /><h1>QUIZ ROYALE</h1><p>{message}</p></div>;
}
